// [사용되지 않는 코드 - 주석 처리됨] securityMatcher("/auth/**")에 걸려있지만
// shouldNotFilter가 보는 "/api/auth/verify" 경로는 이 체인에서 절대 매치되지
// 않고(그 경로는 /api/** 쪽 체인 소관), 그 엔드포인트 자체도 더 이상 존재하지
// 않아 실질적으로 죽은 필터였다. SecurityConfig의 등록부(필드/addFilterBefore)도
// 함께 주석 처리했다 — 필터 체인 구성이 바뀌는 변경이라 별도로 보고함.
